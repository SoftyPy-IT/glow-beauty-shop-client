...
const HeaderSearchBar: React.FC<HeaderSearchBarProps> = ({ onClose }) => {
  ...
  return (
    <div className="fixed inset-0 bg-black/20 backdrop-blur-sm z-50 flex items-start justify-center pt-20">
      <div
        ref={searchRef}
        className="w-full max-w-4xl bg-white shadow-2xl rounded-2xl mx-4 overflow-hidden border border-pink-100"
      >
        {/* Search Input */}
        <div className="p-4 border-b border-pink-100">
          <div className="relative">
            <div className="absolute inset-y-0 left-0 flex items-center pl-4">
              <MagnifyingGlassIcon
                className={`w-5 h-5 transition-colors duration-200 ${isFocused ? "text-rose-500" : "text-pink-400"
                  }`}
              />
            </div>
            ...
            <input
              ref={inputRef}
              type="search"
              className={`w-full py-3 pl-12 pr-12 text-lg bg-pink-50/50 border rounded-xl outline-none transition-all duration-200
                ${isFocused
            ...
            <div className="absolute inset-y-0 right-0 flex items-center space-x-1 pr-2">
              {searchTerm && (
                <button
                  onClick={clearSearch}
                  className="p-2 text-pink-400 hover:text-rose-600 rounded-full hover:bg-pink-100"
                >
        ...
        <div className="max-h-[60vh] overflow-y-auto">
            ...
            <div className="p-4 space-y-3">
              {[1, 2, 3].map((i) => (
                <div key={i} className="flex items-center gap-4">
                  <div className="w-16 h-16 rounded-lg bg-pink-100 animate-pulse" />
                  <div className="flex-1 space-y-2">
                    <div className="h-4 bg-pink-100 rounded animate-pulse w-3/4" />
                    <div className="h-4 bg-pink-100 rounded animate-pulse w-1/4" />
                  </div>
                </div>
              ))}
            </div>
            ...
            <div className="divide-y divide-pink-100">
              {products.data.map((product: IProduct) => (
                <Link
                  ...
                  key={product._id}
                  className="block hover:bg-pink-50 transition-colors"
                  onClick={onClose}
                >
                  <div className="flex items-center p-4 gap-4">
                    <div className="relative w-16 h-16 rounded-lg overflow-hidden bg-pink-100 flex-shrink-0">
                      <Image
                      ...
                      <div className="mt-1 flex items-center gap-2">
                        {product.discount_price > 0 ? (
                          <>
                            ...
                            <span className="text-pink-300 line-through">
          ...
          {!searchTerm && (
            <div className="p-8 text-center text-pink-400">
              <p>Start typing to search for beauty products</p>
            </div>
          )}
        </div>
      </div>
